...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l2_timers_and_sleep-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l2_timers_and_sleep-tq", TaskQueue())
	}
}

// TestApprovalWorkflow — юнит-тест на TestWorkflowEnvironment: активности
...
func TestApprovalWorkflow(t *testing.T) {
	...
	env.OnActivity(Poll, mock.Anything).Return("ok", nil)
	env.OnActivity(NotifyDecision, mock.Anything).Return("ok", nil)
	env.ExecuteWorkflow(ApprovalWorkflow)
...
func TestIntegration(t *testing.T) {
	...
	// стартани ApprovalWorkflow через client.ExecuteWorkflow и проверь результат
	// поведения урока «Таймеры и Sleep».
}
